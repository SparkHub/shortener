...
describe Shortener::ShortenerHelper, type: :helper do
  describe '#short_url' do
    ...
    context 'without user or custom key' do
      before do
        expect(Shortener::ShortenedUrl).to receive(:generate).with(destination, owner: nil, custom_key: nil, expires_at: nil, fresh: false, meta: nil).and_return(shortened_url)
      end
    ...
    context 'with owner' do
      ...
      it 'sends user to generate function' do
        expect(Shortener::ShortenedUrl).to receive(:generate).with(destination, owner: owner, custom_key: nil, expires_at: nil, fresh: false, meta: nil)
    ...
    context 'with custom_key' do
      it 'sends custom key code to generate function' do
        expect(Shortener::ShortenedUrl).to receive(:generate).with(destination, owner: nil, custom_key: 'custkey', expires_at: nil, fresh: false, meta: nil)
    ...
    context 'with expires_at' do
      it 'sends custom key code to generate function' do
        expect(Shortener::ShortenedUrl).to receive(:generate).with(destination, owner: nil, custom_key: nil, expires_at: 'testtime', fresh: false, meta: nil)
