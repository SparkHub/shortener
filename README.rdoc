{<img src="https://secure.travis-ci.org/SparkHub/shortener.png" alt="Build Status" />}[http://travis-ci.org/SparkHub/shortener] {<img src="https://codeclimate.com/github/SparkHub/shortener/badges/gpa.svg" />}[https://codeclimate.com/github/SparkHub/shortener]
...
Since 1.1.0[https://github.com/SparkHub/shortener/releases/tag/v1.1.0], breaking changes have been included:
- Ruby +2.0.0+ and +2.1.0+ are no longer supported.
- +MySQL+ is no longer supported and have been replaced by +Postgres+.
...
By default, the shortener counter will be incremented even if a bot/crawler access the shortened URL. You can disable this
behavior with:
...
You can include a callback and perform some business logic and/or also modify final url (e.g. add additional query params), by including the following:

  # Include all business logic and final url before redirection
  #
  # options = { short_url: {url: <the original url>, shortened_url: <the retrieved ShortenedURL>}, request: <request> }
  #
  # Returns a url to perform the redirection
  #
  Shortener.hook_handler = proc do |options|
    # biz(...)
    options[:short_url][:url]
  end
...
=== Shortening URLS out of controller/views (still need a request)

A request must be given to the shortened method in order to retrieve the host/protocol/port

  include Shortener::ShortenerHelper

  short_url('http://example.com', req: request)
...
=== Shorten URLs with meta parameters <em>(PostgreSQL only)</em>

These extra information will not be added to the final URL. They are added for analytics purposes or any other type of tagging.

First, add a field +:meta+ in the migration. This column needs to be a +:hstore+ type

  def change
    add_column :shortened_urls, :meta, :hstore
  end

Then enable it in the Shortener configuration

  # config/initializers/shortener.rb
  Shortener.enable_meta = true

Bonus: add an index if you plan on querying this +:meta+ column
...
=== Shorten URLs with related_id and source_type parameters
Additional columns were added to a shortened URL:

+related_id+: includes object's id linking (an object with a url)

+source_type+: includes a tag to distinguish or categorize the url

These values are stored in two columns in the database, which can be added to your schema with the following
migration:

  bundle exec rails g migration add_additional_columns_to_shortened_url
