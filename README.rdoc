...
=== Shorten URLs with message_id, source and campaign_user_id parameters
v1.0.3 introduced more columns to a shortened URL. The message_id, source and campaign_user_id values
are stored in a string/uuid column in the database, which can be added to your schema with the following
migration:

Add the fields in the migration. Columns +message_id+ and +campaign_user_id+ need to be a +:uuid+ type

Data type +:uuid+ is not supported natively for sqlite
  ...
  class AddAdditionalColumnsToShortenedUrl < ActiveRecord::Migration
    def change
      add_column :shortened_urls, :message_id, :uuid, index: true
      add_column :shortened_urls, :source, :string, limit: 50
      add_column :shortened_urls, :campaign_user_id, :uuid, index: true
    end
  end
